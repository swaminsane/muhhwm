...
static void topstrip_init(Module *m, int x, int y, int w, int h) {
  (void)x;
  (void)y;
  m->w = w;
  m->h = h;
...
static void topstrip_draw(Module *m, int x, int y, int w, int h, int focused) {
  if (m->theme) {
  ...
  const char *label = "Top Strip";
  int tw = drw_fontset_getwidth(drw, label);
  drw_setscheme(drw, scheme[0]);
  drw_text(drw, x + pad, y + (h - font_h) / 2, tw, font_h, 0, label, 0);
}

static void topstrip_input(Module *m, const InputEvent *ev) { /* placeholder */
}
