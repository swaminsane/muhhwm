...
static void kde_notif_init(Module *m, int x, int y, int w, int h) {
  (void)x;
  (void)y;
  m->w = w;
  m->h = h;
...
static void kde_notif_draw(Module *m, int x, int y, int w, int h, int focused) {
  if (m->theme) {
  ...
  const char *label = "KDE Notifications";
  int tw = drw_fontset_getwidth(drw, label);
  drw_setscheme(drw, scheme[0]);
  drw_text(drw, x + pad, y + (h - font_h) / 2, tw, font_h, 0, label, 0);
}

static void kde_notif_input(Module *m, const InputEvent *ev) { /* placeholder */
}

Module kde_notifications_module = {
    .name = "kde_notifications",
    .init = kde_notif_init,
    .draw = kde_notif_draw,
    .input = kde_notif_input,
...
void __attribute__((constructor)) kde_notifications_register(void) {
  register_module(&kde_notifications_module);
}
