...
static void bottom_init(Module *m, int x, int y, int w, int h) {
  (void)x;
  (void)y;
  m->w = w;
  m->h = h;
...
static void bottom_draw(Module *m, int x, int y, int w, int h, int focused) {
  if (m->theme) {
...
static void bottom_input(Module *m, const InputEvent *ev) { /* placeholder */ }

static LayoutHints *bottom_hints(Module *m) {
