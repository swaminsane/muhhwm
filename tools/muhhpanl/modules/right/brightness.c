...
static void brightness_init(Module *m, int x, int y, int w, int h) {
  (void)x;
  (void)y;
  m->w = w;
  m->h = h;
...
static void brightness_draw(Module *m, int x, int y, int w, int h,
                            int focused) {
  if (m->theme) {
...
static void brightness_input(Module *m,
                             const InputEvent *ev) { /* placeholder */ }

static LayoutHints *brightness_hints(Module *m) {
...
Module brightness_module = {
    ...
    .input = brightness_input,
    .get_hints = brightness_hints,
