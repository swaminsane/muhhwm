...
static void volume_init(Module *m, int x, int y, int w, int h) {
  (void)x;
  (void)y;
  m->w = w;
  m->h = h;
...
static void volume_draw(Module *m, int x, int y, int w, int h, int focused) {
  if (m->theme) {
...
static void volume_input(Module *m, const InputEvent *ev) { /* placeholder */ }

static LayoutHints *volume_hints(Module *m) {
