...
typedef struct {
  ...
  int dragging; /* 1 = currently dragging, 0 = idle */
  int drag_start_x;
  int drag_start_val;
...
/* allocate an XftColor from a colour string */
...
static void vol_draw(Module *m, int x, int y, int w, int h, int focused) {
  ...
  int label_y = y + pad + (24 - font_h) / 2;
  drw_setscheme(drw, s->label_scheme);
  drw_text(drw, x + pad, label_y, max_label_w, font_h, 0, label, 0);
  ...
  if (track_len < 16)
    track_len = 16;
  int knob_size = 10;
  int knob_x = track_start + (s->volume / 100.0f * track_len) - (knob_size / 2);
  ...
  /* track before knob */
  XFillRectangle(dpy, drw->drawable, drw->gc, track_start, y + pad + 24 / 2 - 1,
                 knob_x - track_start, 2);
  /* remaining track */
  XSetForeground(dpy, drw->gc, 0x3b4252);
  XFillRectangle(dpy, drw->drawable, drw->gc, knob_x + knob_size,
                 y + pad + 24 / 2 - 1, track_end - (knob_x + knob_size), 2);

  /* knob */
  int knob_y = y + pad + (24 - knob_size) / 2;
...
// … (everything before vol_input stays the same) …
...
static void vol_input(Module *m, const InputEvent *ev) {
  ...
  // … (release, right‑click remain unchanged) …
}
