...
static void pomodoro_init(Module *m, int x, int y, int w, int h) {
  (void)x;
  (void)y;
  m->w = w;
  m->h = h;
...
static void pomodoro_draw(Module *m, int x, int y, int w, int h, int focused) {
  if (m->theme) {
...
static void pomodoro_input(Module *m, const InputEvent *ev) { /* placeholder */
}
