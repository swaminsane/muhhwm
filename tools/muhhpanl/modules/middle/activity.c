...
static void activity_init(Module *m, int x, int y, int w, int h) {
  (void)x;
  (void)y;
  m->w = w;
  m->h = h;
...
static void activity_draw(Module *m, int x, int y, int w, int h, int focused) {
  if (m->theme) {
...
static void activity_input(Module *m, const InputEvent *ev) { /* placeholder */
}
...
void __attribute__((constructor)) activity_register(void) {
  register_module(&activity_module);
}
