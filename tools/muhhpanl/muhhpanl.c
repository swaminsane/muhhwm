...
/* ── font & colour arrays (only used here) ─────── */
...
/* ── layout tree (moved from panel.h) ───────────── */
LayoutNode layout_tree =
{
    .type = LAYOUT_COL,
    .children = (LayoutNode[]){
        /* Row 0 – topstrip */
        {.type = LAYOUT_ROW, .fixed_px = 20,
         .children = (LayoutNode[]){{.type = LAYOUT_MODULE, .module_name = M_TOPSTRIP, .weight = 1.0f}, {.type = 0}}, .nchildren = 1},
        /* Row 1 – main area */
        {.type = LAYOUT_ROW, .weight = 70.0f,
         .children = (LayoutNode[]){
             /* left subpanel 67% */
             {.type = LAYOUT_COL, .weight = 63.0f,
              .children = (LayoutNode[]){
                  /* top 65% → 50/50 split */
                  {.type = LAYOUT_ROW, .weight = 60.0f,
                   .children = (LayoutNode[]){
 ...
 .children = (LayoutNode[]){
     ...
     /* ── top sub‑panel (35%) – daystrip, calendar, pomodoro ── */
     ...
     {.type = LAYOUT_COL, .weight = 13.0f,
      .children = (LayoutNode[]){
          /* spacer to push the two bars to the bottom */
          {.type = LAYOUT_COL, .weight = 1.0f,
           .children = (LayoutNode[]){{.type = 0}}, .nchildren = 0},
          ...
          {.type = LAYOUT_COL, .fixed_px = 54,
           ...
           .children = (LayoutNode[]){
               {.type = LAYOUT_MODULE, .module_name = M_MID_TEXTSMENU,
                .fixed_px = 24, .margin_top = 0, .margin_bottom = 3},
               {.type = LAYOUT_MODULE, .module_name = M_SMS_INPUT,
                .fixed_px = 24, .margin_top = 3, .margin_bottom = 0},
               {.type = 0}}, .nchildren = 2},
          {.type = 0}}, .nchildren = 2},

     {.type = 0}}, .nchildren = 3},   /* end of middle column */
                       {.type = 0}}, .nchildren = 2},
                  /* bottom 35% → profanity */
                  {.type = LAYOUT_COL, .weight = 40.0f,
              ...
              .children = (LayoutNode[]){
                  ...
                  {.type = LAYOUT_COL, .weight = 55.0f,
                   .children = (LayoutNode[]){
                       ...
                       {.type = LAYOUT_COL, .weight = 40.0f,
                        .children = (LayoutNode[]){
                            ...
                            {.type = LAYOUT_ROW, .fixed_px = 112,
                             .children = (LayoutNode[]){
                                 {.type = LAYOUT_COL, .weight = 1.0f, .children = (LayoutNode[]){{.type = 0}}, .nchildren = 0},
                                 {.type = LAYOUT_COL, .fixed_px = 250,
                                  .children = (LayoutNode[]){{.type = LAYOUT_MODULE, .module_name = M_RIGHT_CLOCK, .weight = 1.0f}, {.type = 0}}, .nchildren = 1},
                                 {.type = LAYOUT_COL, .fixed_px = 250,
                       ...
                       {.type = LAYOUT_COL, .weight = 25.0f, .margin_top = 4,
                        .children = (LayoutNode[]){
                            ...
                            {.type = LAYOUT_COL,
                             ...
                             .expand_y = 0,
                             .children = (LayoutNode[]){
                                 {.type = LAYOUT_MODULE, .module_name = M_RIGHT_VOL, .fixed_px = 24,
                                  .margin_top = 0, .margin_bottom = 0},
                                 {.type = LAYOUT_MODULE, .module_name = M_RIGHT_BRI, .fixed_px = 50,
                                  .margin_top = 0, .margin_bottom = 0},
                                 {.type = 0}},
                             .nchildren = 2},
                       ...
                       {.type = 0}}, .nchildren = 3},     /* end of 3 sub‑panels */

                  /* ── bottom half (45%): video box + search bar ── */
                  {.type = LAYOUT_COL, .weight = 45.0f,
                   .children = (LayoutNode[]){
                       {.type = LAYOUT_MODULE, .module_name = M_MPVBOX, .weight = 1.0f},
                       {.type = LAYOUT_MODULE, .module_name = M_MPVSEARCH, .fixed_px = 24},
                       {.type = 0}}, .nchildren = 2},
                  {.type = 0}}, .nchildren = 2},          /* end of right subpanel */
             {.type = 0}}, .nchildren = 2},
        /* Row 2 – timeline */
        {.type = LAYOUT_ROW, .fixed_px = 13,
         .children = (LayoutNode[]){{.type = LAYOUT_MODULE, .module_name = M_TIMELINE, .weight = 1.0f}, {.type = 0}}, .nchildren = 1},
        {.type = 0}},
    .nchildren = 3
};
...
static int get_bar_height(void) {
    ...
    Atom type; int format; unsigned long n, after;
...
static void update_geometry(void) {
    ...
    if (panel_x + panel_w > sw) panel_x = sw - panel_w;
    if (panel_x < 0) panel_x = 0;
    panel_h = (sh * panel_height_pct) / 100;
    if (panel_h > sh - barh) panel_h = sh - barh;
}
...
static void show_panel(void) {
    ...
    animating = 1; anim_step = 0; anim_dir = 1;
    panel_y = -panel_h;
    move_panel(panel_y);
    mpvbox_show();
    profanity_show();
...
/* ── drawing ────────────────────────────────────── */
static void draw_panel(void) {
    if (!panel_shown && !animating) return;
...
/* call after a module changes its own size hints */
    ...
    while (1) {
        ...
        while (XPending(dpy)) {
            ...
            if (translate_event(&ev, &iev)) {
                if (!panel_shown) continue;

                /* mpv event replay */
                if (mpvbox_w > 0 && iev.root_x >= mpvbox_x && iev.root_x < mpvbox_x + mpvbox_w &&
                    iev.root_y >= mpvbox_y && iev.root_y < mpvbox_y + mpvbox_h) {
                ...
                /* profanity event replay */
                if (profanity_w > 0 && iev.root_x >= profanity_x && iev.root_x < profanity_x + profanity_w &&
                    iev.root_y >= profanity_y && iev.root_y < profanity_y + profanity_h) {
                    if (iev.type == EV_PRESS && profanity_win_export) {
                        XAllowEvents(dpy, ReplayPointer, CurrentTime);
                        continue;
                    }
                    if (iev.type == EV_KEY_PRESS && profanity_win_export) {
                        continue;   /* terminal receives events directly now */
                    }
                }
                ...
                /* Escape hides */
                if (iev.type == EV_KEY_PRESS && XLookupKeysym(&ev.xkey, 0) == XK_Escape) {
                    hide_panel();
                    continue;
                }
...
int main(void) {
    ...
    panel_win = XCreateWindow(dpy, root, panel_x, -panel_h, panel_w, panel_h, 0,
